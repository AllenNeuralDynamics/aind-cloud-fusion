...
class TestDatasetReal(io.Dataset):
    ...
    def tile_volumes_tczyx(self) -> dict[int, io.LazyArray]:
        ...
        tile_volumes = {5: da.from_zarr(tile_5),
                        6: da.from_zarr(tile_6)}
        return tile_volumes

    @property
    def tile_transforms_zyx(self) -> dict[int, list[geometry.Transform]]:
        tile_5_transforms = [geometry.Affine(np.array([[1., 0., 0., 93.0],
                                                       [0., 1., 0., 1851.0],
                                                       [0., 0., 1., 5554.0]])),
                             geometry.Affine(np.array([[1., 0., 0., 4.34549],
                                                       [0., 1., 0., 3.67476],
                                                       [0., 0., 1., -65.9757]]))]
        tile_6_transforms = [geometry.Affine(np.array([[1., 0., 0., 114.0],
                                                       [0., 1., 0., 1851.0],
                                                       [0., 0., 1., 3703.0]])),
                             geometry.Affine(np.array([[1., 0., 0., 1.78407],
                                                       [0., 1., 0., -16.18333],
                                                       [0., 0., 1., -46.52276]]))]
...
def generate_real_dataset():
    ground_truth = zarr.open("tests/real_data/fused.zarr/0")  # Full resolution was fused from downsampled inputs.
